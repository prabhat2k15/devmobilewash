...
'agent_id'=> $washrequest_id_check->agent_id,
		...
		$washerdropjobs =  Yii::app()->db->createCommand("SELECT COUNT(*) as count FROM activity_logs WHERE agent_id = ".$washrequest_id_check->agent_id." AND action = 'dropjob'")->queryAll();
				...
				if(!is_numeric($agent_feedback->customer_ratings)) $rate += 5;
                     ...
                     if($washer_total_dropjobs) $agent_rate =  ($rate + $washer_total_dropjobs) / ($total_rate + 10 + $washer_total_dropjobs);
		    else $agent_rate =  $rate/($total_rate + 10);
                    $agent_rate = number_format($agent_rate, 2, '.', '');
					  ...
					  //if($prequest['order_temp_assigned'] == 0 AND (!$order_rejects)){
                       //if($prequest['order_temp_assigned'] == 0){
						  /* ------- check if agent is nearest -------- */
							...
							$data = array("customer_id"=>$prequest['customer_id'], "wash_request_id"=>$prequest['id'], "agent_id"=>$agent_id, "api_password" => AES256CBC_API_PASS, "key" => API_KEY);
							curl_setopt($handle, CURLOPT_POST, true);
							curl_setopt($handle, CURLOPT_POSTFIELDS, $data);
							curl_setopt($handle,CURLOPT_RETURNTRANSFER,1);
		...
		$addonscompleted = 0;
                ...
                if(count($val['total_cars'])>0){
                 ...
                 if(count($val['pending'])>0 || count($val['processing']) >0 || count($val['complete'])>0 || count($val['canceled'])>0 || count($val['scheduled_auto'])>0 || count($val['ondemandautocanceled'])>0 ){
                    $total_orders = count($val['pending'])+count($val['processing'])+count($val['complete'])+count($val['canceled'])+count($val['scheduled_auto'])+count($val['ondemandautocanceled']);
