...
class SiteController extends Controller {
    ...
    public function actionaddreminder() {
        ...
        $client_reminder = '';
        if (Yii::app()->request->getParam('client_reminder'))
            $client_reminder = Yii::app()->request->getParam('client_reminder');

        if (Yii::app()->request->getParam('agent_reminder'))
            $agent_reminder = Yii::app()->request->getParam('agent_reminder');
    ...
    public function actionadminpendingschedwashesalert() {
        ...
        if (count($pendingwashes)) {
            foreach ($pendingwashes as $schedwash) {
                ...
                if ($min_diff > 0 && $min_diff <= 15)
                    $flaggedwashids[] = $schedwash->id;
            }
            ...
            if (!count($wrequest_id_check)) {
                ...
                if ($admin_command == 'save-status') {
                    ...
                    if ($status == 1)
                        $actionname = 'startjob';
                    if ($status == 2)
                        $actionname = 'arrivejob';
                    if ($status == 3)
                        $actionname = 'processjob';
                    if ($status == 4)
        ...
        if ((isset($id) && !empty($id))) {
            ...
            if (!count($item_check)) {
                ...
                if ($action == 'disablesurge') {
                    ...
                    if ($item_check[0]['zipcodes'])
                        $new_zips = $item_check[0]['zipcodes'] . "," . $zip;
                    else
                        $new_zips = $zip;
    ...
    public function actiongetallwashrequestsnew() {
        ...
        if (!empty(Yii::app()->request->getParam('event'))) {
            ...
            if ($event == 'pending') {
                ...
                $to = Yii::app()->request->getParam('to');
                $agent_id = Yii::app()->request->getParam('agent_id');
                $order_day = " (w.agent_id = " . $agent_id . ") AND DATE_FORMAT(w.order_for,'%Y-%m-%d') BETWEEN '" . $from . "' AND '" . $to . "' AND status IN (4,5,7)";
            } else {
                $status_qr = '';
            }
...
//        if ($event == "completed") {
//            $order_day = " AND DATE_FORMAT(w.complete_order,'%Y-%m-%d')= '" . $day . "'";
//        }
...
//else $qrRequests =  Yii::app()->db->createCommand("SELECT * FROM washing_requests WHERE wash_request_position = 'real' ".$order_day." ORDER BY id DESC")->queryAll();
        ...
        elseif ($event == 'declined') {
            ...
            if ($limit > 0)
                ...
                $qrRequests = Yii::app()->db->createCommand("SELECT w.* FROM washing_requests w LEFT JOIN customers c ON w.customer_id = c.id  LEFT JOIN agents a ON w.agent_id = a.id WHERE " . $cust_query . $agent_query . " c.hours_opt_check = 1 AND w.wash_request_position = '" . APP_ENV . "' " . $order_day . " ORDER BY w.id DESC")->queryAll();
        }
        ...
        if (count($qrRequests) > 0) {
            ...
            foreach ($qrRequests as $ind => $wrequest) {
                ...
                if ($event == 'scheduleauto') {
                    ...
                    $ondemandautocanceled = Yii::app()->db->createCommand("SELECT * FROM activity_logs WHERE action = 'scheduleauto-canceled' AND wash_request_id = :order_id")->bindValue(':order_id', $wrequest['id'], PDO::PARAM_STR)->queryAll();
...
//else $qrRequests =  Yii::app()->db->createCommand("SELECT * FROM washing_requests WHERE wash_request_position = 'real' ".$order_day." ORDER BY id DESC")->queryAll();
        ...
        if ($filter == 'testorders') {
            ...
            if ($limit > 0)
                $qrRequests = Yii::app()->db->createCommand("SELECT w.* FROM washing_requests w LEFT JOIN customers c ON w.customer_id = c.id WHERE " . $cust_query . $agent_query . "c.hours_opt_check = 0 AND w.wash_request_position = '" . APP_ENV . "' " . $order_day . " ORDER BY w.id DESC LIMIT " . $limit . " OFFSET " . $offset)
                        ->bindValue(':customer_id', $customer_id, PDO::PARAM_STR)
                        ->bindValue(':agent_id', $agent_id, PDO::PARAM_STR)
                        ->queryAll();
        }
            ...
            if ($limit > 0)
                $qrRequests = Yii::app()->db->createCommand("SELECT w.* FROM washing_requests w LEFT JOIN customers c ON w.customer_id = c.id WHERE " . $cust_query . $agent_query . "c.hours_opt_check = 1 AND w.wash_request_position = '" . APP_ENV . "' " . $order_day . " ORDER BY w.id DESC LIMIT " . $limit . " OFFSET " . $offset)
                        ->bindValue(':customer_id', $customer_id, PDO::PARAM_STR)
                        ->bindValue(':agent_id', $agent_id, PDO::PARAM_STR)
                        ->queryAll();
        }
...
//echo $wrequest['id']." ".$wrequest['order_for']." "."<br>";
            ...
            foreach ($qrRequests as $ind => $wrequest) {
                ...
                if ($wrequest['failed_transaction_id']) {
                    ...
                    if ($wrequest['transaction_id']) {
                        ...
                        if ($cust_details->client_position == 'real')
                            $payresult = Yii::app()->braintree->getTransactionById_real($wrequest['transaction_id']);
                        else
                            $payresult = Yii::app()->braintree->getTransactionById($wrequest['transaction_id']);
                        if ($payresult['success'] == 1) {
                            //$submerchant_id = $payresult['merchant_id'];
                            $transaction_status = $payresult['status'];
                        }
    ...
    public function actionsearchorders() {
        ...
        if (!$search_area)
            $search_area = "Order Number";

        if ($search_area == "Order Number")
            $order_query = "(id LIKE :query) ";
        if ($search_area == "Created Date")
            $order_query = "(created_date LIKE :query) ";
        if ($search_area == "Scheduled Date")
            $order_query = "(order_for LIKE :query AND is_scheduled = 1) ";
        if ($search_area == "On-Demand")
            $order_query = "(is_scheduled = 0) ";
...
//$message = str_replace("[CUSTNAME]",$custname, $message);
                    ...
                    if ($cust_details->email)
                        Vargas::Obj()->SendMail($cust_details->email, $emailfrom, $emailbody, $subject, 'mail-receipt');
...
//echo "<br>";
    ...
    public function actionnonreturncustomercheck() {
        ...
        $clientlist = Customers::model()->findAllByAttributes(array('non_return_check' => 1), array('limit' => 500));
        ...
        if (count($clientlist)) {
            ...
            if ((count($inactive5_notify_check)) && (!$inactive5_notify_check[0]['topic_arn'])) {
                ...
                try {
                    ...
                    $inactive5_topic_arn = $aws_topic_result['TopicArn'];
                } catch (exception $e) {
                    
                }

                Yii::app()->db->createCommand("UPDATE customer_spec_notifications SET topic_arn = :topic_arn WHERE notify_cat = :notify_cat")
                        ->bindValue(':topic_arn', $inactive5_topic_arn, PDO::PARAM_STR)
                        ->bindValue(':notify_cat', 'inactive-6th-day', PDO::PARAM_STR)
                        ->execute();
            } else {
            ...
            if ((count($inactive10_notify_check)) && (!$inactive10_notify_check[0]['topic_arn'])) {
                ...
                try {
                    ...
                    $inactive10_topic_arn = $aws_topic_result['TopicArn'];
                } catch (exception $e) {
                    
                }

                Yii::app()->db->createCommand("UPDATE customer_spec_notifications SET topic_arn = :topic_arn WHERE notify_cat = :notify_cat")
                        ->bindValue(':topic_arn', $inactive10_topic_arn, PDO::PARAM_STR)
                        ->bindValue(':notify_cat', 'inactive-11th-day', PDO::PARAM_STR)
                        ->execute();
            } else {
            ...
            if ((count($inactive30_notify_check)) && (!$inactive30_notify_check[0]['topic_arn'])) {
                ...
                try {
                    ...
                    $inactive30_topic_arn = $aws_topic_result['TopicArn'];
                } catch (exception $e) {
                    
                }

                Yii::app()->db->createCommand("UPDATE customer_spec_notifications SET topic_arn = :topic_arn WHERE notify_cat = :notify_cat")
                        ->bindValue(':topic_arn', $inactive30_topic_arn, PDO::PARAM_STR)
                        ->bindValue(':notify_cat', 'inactive-31st-day', PDO::PARAM_STR)
                        ->execute();
            } else {
                $inactive30_topic_arn = $inactive30_notify_check[0]['topic_arn'];
            }
...
//more than 30 days
    ...
    public function actiongetnonreturncustomers() {
        ...
        if ($limit > 0)
            $nonreturncust_arr_30 = Yii::app()->db->createCommand("SELECT id, first_name, last_name, email, contact_number, total_wash FROM customers WHERE is_non_returning = 1 AND nonreturn_cat = 30 ORDER BY id ASC LIMIT " . $limit . " OFFSET " . $offset_30)->queryAll();
        ...
        if ($limit > 0)
            $nonreturncust_arr_60 = Yii::app()->db->createCommand("SELECT id, first_name, last_name, email, contact_number, total_wash FROM customers WHERE is_non_returning = 1 AND nonreturn_cat = 60 ORDER BY id ASC LIMIT " . $limit . " OFFSET " . $offset_60)->queryAll();
        ...
        if ($limit > 0)
            $nonreturncust_arr_90 = Yii::app()->db->createCommand("SELECT id, first_name, last_name, email, contact_number, total_wash FROM customers WHERE is_non_returning = 1 AND nonreturn_cat = 90 ORDER BY id ASC LIMIT " . $limit . " OFFSET " . $offset_90)->queryAll();
    ...
    public function actionwashfraudcheck() {
        ...
        if (count($all_washes)) {
            ...
            foreach ($all_washes as $ind => $wash) {
                ...
                $good_card = 0;
                if ($cust_detail->client_position == 'real')
                    $Bresult = Yii::app()->braintree->getCustomerById_real($cust_detail->braintree_id);
                else
                    $Bresult = Yii::app()->braintree->getCustomerById($cust_detail->braintree_id);
                ...
                if (count($last_wash)) {
                    ...
                    if ($wash['address'] != $last_wash->address)
                        $is_flagged = 1;
                }
		...
		if($wrequest_id_check->canceled_wash_transaction_id){
			...
			if ($customer_check->client_position == 'real') $voidresult = Yii::app()->braintree->void_real($wrequest_id_check->canceled_wash_transaction_id);
                ...
                if (strtotime($wrequest_id_check->wash_begin) > 0) {
                    ...
                    Washingrequests::model()->updateByPk($wash_request_id, array("status" => 0, "agent_id" => 0, "wash_begin" => date('Y-m-d H:i:s'), "washer_late_cancel" => 0, "no_washer_cancel" => 0, "company_cancel" => 0, "cancel_fee" => 0, "washer_cancel_fee" => 0, 'washer_payment_status' => 0, 'canceled_wash_transaction_id' => ''));
                } else {

                    Washingrequests::model()->updateByPk($wash_request_id, array("status" => 0, "agent_id" => 0, "washer_late_cancel" => 0, "created_date" => date('Y-m-d H:i:s'), "no_washer_cancel" => 0, "company_cancel" => 0, "cancel_fee" => 0, "washer_cancel_fee" => 0, 'washer_payment_status' => 0, 'canceled_wash_transaction_id' => ''));
                }
                ...
                if (!$token) {
                    ...
                    if (($wrequest_id_check->agent_id != $agent_id) && ($result != 'false')) {
                        ...
                        if ((!$wrequest_id_check->is_scheduled)) {
                            if (($agent_id))
                                Washingrequests::model()->updateByPk($wash_request_id, array("agent_id" => $agent_id, 'washer_payment_status' => 0, 'status' => 1, 'wash_begin' => date('Y-m-d H:i:s')));
                            else
                                Washingrequests::model()->updateByPk($wash_request_id, array("agent_id" => $agent_id, 'washer_payment_status' => 0, 'status' => 0, 'order_temp_assigned' => 0));
                        } else
                        ...
                        if ($agent_id) {
                            ...
                            if ($wrequest_id_check->is_scheduled)
                                $message = "SCHEDULED WASH TAKEN ";
                            else
                                $message = "WASH NOW TAKEN ";
    ...
    public function actionupdatedevicestatus() {
        ...
        if ((isset($user_type) && !empty($user_type)) && (isset($user_id) && !empty($user_id)) && (isset($device_token) && !empty($device_token))) {
            ...
            if (!count($user_check)) {
                ...
                if ($user_type == 'customer') {
                    if ($user_check->forced_logout == 1 || $user_check->block_client == 1)
                        $response = 'offline';
                    else
                        $response = 'online';
                ...
                if ($user_type == 'agent') {
                    ...
                    if ($user_rating > 5) {
                        ...
                        if ($agent_rate < 3)
                            $agentmodel->updateAll(array("rating" => $agent_rate, "block_washer" => 1, "forced_logout" => 1), 'id=:id', array(':id' => $user_check->id));
                        else
                            $agentmodel->updateAll(array("rating" => $agent_rate), 'id=:id', array(':id' => $user_check->id));
...
//var_dump($geojsondata);
    ...
    public function actionprewasherexport() {
        ...
        CsvExport::export(
                ...
                array('id' => array('raw'), 'first_name' => array('text'), 'last_name' => array('text'), 'email' => array('text'), 'phone' => array('text'), 'city' => array('text'), 'state' => array('text'), 'zipcode' => array('text'), 'hear_mw_how' => array('text'), 'van_lease' => array('text'), 'register_date' => array('date')), true, // boolPrintRows
    ...
    public function actionnonreturncustscsvexport() {
        ...
        if ($limit > 0)
            $all_customers = Yii::app()->db->createCommand("SELECT * FROM customers WHERE is_non_returning = 1 ORDER BY id ASC LIMIT " . $limit . " OFFSET " . $offset)->queryAll();
        ...
        CsvExport::export(
                ...
                array('id' => array('raw'), 'customername' => array('text'), 'email' => array('text'), 'phone' => array('text'), 'total_wash' => array('text'), 'last_order' => array('text')), true, // boolPrintRows
    ...
    public function actioninactivecustscsvexport() {
        ...
        CsvExport::export(
                ...
                array('id' => array('raw'), 'customername' => array('text'), 'email' => array('text'), 'phone' => array('text'), 'last_activity' => array('text')), true, // boolPrintRows
            ...
            foreach ($yelp_reviews->reviews as $yreview) {
                if ($yreview->rating == 5) {
                    ...
                    if (!count($review_check))
                        Yii::app()->db->createCommand("INSERT INTO `mobilewash_reviews` (`review_org_id`, `reviewer_name`, `reviewer_location`, `reviewer_photo`, `review`, `rating`, `review_date`, `review_url`, `review_source`) VALUES ('" . $yreview->id . "', '" . $yreview->user->name . "', '', '" . $yreview->user->image_url . "', '" . $yreview->text . "', '" . $yreview->rating . "', '" . $yreview->time_created . "', '" . $yreview->url . "', 'yelp'); ")->execute();
                }
            }
        }
...
//print_r($phone_number->carrier['type']);
                ...
                try {
                    ...
                    if (count($phone_number)) {
                        if ($phone_number->carrier['type'] == 'voip')
                            Customers::model()->updateByPk($customer['id'], array("is_voip_number" => 1));
                    }
        ...
        if (count($all_agents)) {
            foreach ($all_agents as $agent) {
                ...
                try {
                    ...
                    if (count($phone_number)) {
                        if ($phone_number->carrier['type'] == 'voip')
                            Agents::model()->updateByPk($agent['id'], array("is_voip_number" => 1));
                    }
                } catch (Twilio\Exceptions\RestException $e) {
                    //echo  $e;
                }
    ...
    public function actionsendcustomerschedulepushnotify() {
        ...
        if (count($allclients) && count($pendingjob)) {
            ...
            if (count($pendingjob))
                Yii::app()->db->createCommand("UPDATE scheduled_notifications SET status=1 WHERE id=" . $pendingjob[0]['id'])->execute();
        }
    }
    ...
    public function actionfixcustnameparts() {
        ...
        if (count($all_custs)) {
            foreach ($all_custs as $cust) {
                ...
                echo "<br><br>";
                if (($updateentry == 1) && (count($custname_parts) > 0))
                    Customers::model()->updateByPk($cust['id'], array("first_name" => trim($custname_parts[0], " "), "last_name" => trim($custname_parts[1], " ")));
            }
        }
        else {
            echo "nothing found";
    ...
    public function actiongetphonefortwiliocall() {
        ...
        if (count($customer)) {
            ...
            if (count($inprocess_wash_check)) {
                ...
                if (count($current_agent))
                    $tophone = $current_agent->phone_number;
            }
        }
        ...
        if (count($agent)) {
            ...
            if (count($inprocess_wash_check)) {
                ...
                if (count($current_cust))
                    $tophone = $current_cust->contact_number;
            }
            else {
...
//        }
        ...
        if (!empty(Yii::app()->request->getParam('event'))) {
            ...
            if ($event == 'pending') {
                ...
                $to = Yii::app()->request->getParam('to');
                $agent_id = Yii::app()->request->getParam('agent_id');
                $order_day = " w.agent_id = '" . $agent_id . "' AND DATE_FORMAT(w.order_for,'%Y-%m-%d') BETWEEN '" . $from . "' AND '" . $to . "' AND status = 4";
            } else {
                $status_qr = '';
            }
        ...
        $limit = 0;
        $filter = Yii::app()->request->getParam('filter');
        $flaggedIssueStatus = Yii::app()->request->getParam('flaggedIssueStatus');
        $limit = Yii::app()->request->getParam('limit');
        $customer_id = Yii::app()->request->getParam('customer_id');
        $agent_id = Yii::app()->request->getParam('agent_id');
        $admin_username = '';
        ...
        if ($flaggedIssueStatus) {
            $flag_query = "w.flagged_issue_status=" . $flaggedIssueStatus . " ";
        } else {
            $flag_query = " w.flagged_issue_status IN(1,2) ";
        }
        if ($filter == 'flaggedIssues') {
            if ($limit > 0) {
                // $qrRequests = Yii::app()->db->createCommand("SELECT w.* FROM washing_requests w LEFT JOIN customers c ON w.customer_id = c.id WHERE " . $cust_query . $agent_query . $flag_query . "c.hours_opt_check = 0 AND w.wash_request_position = '" . APP_ENV . "' " . $order_day . " ORDER BY w.id DESC LIMIT " . $limit)->queryAll();
                $qrRequests = Yii::app()->db->createCommand("SELECT w.* FROM washing_requests w LEFT JOIN customers c ON w.customer_id = c.id WHERE " . $cust_query . $agent_query . $flag_query . "  ORDER BY w.id DESC LIMIT " . $limit)->queryAll();
            } else {
...
//echo $wrequest['id']." ".$days_between."<br>"; 
                ...
                if (($min_diff < 0) && ($wrequest['status'] == 0)) {
                    ...
                    $pendingwashrequests_flashing[] = array('id' => $wrequest['id'],
                        ...
                        'washer_wash_activity' => $washer_wash_activity,
                        'washer_30_min_noarrive' => $washer_30_min_noarrive,
                        'flagged_issue_status' => $wrequest['flagged_issue_status'],
                    );
                }
