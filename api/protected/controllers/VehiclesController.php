...
class VehiclesController extends Controller {
    ...
    public function actionAddVehicle() {
        ...
        if ((isset($vehicle_build) && !empty($vehicle_build)) &&
            ...
            if ($resIns) {
                $result = 'true';
                if ($vehicle_build == 'classic')
                    $response = 'Classic vehicle added successfully';
                else
                    $response = 'Regular vehicle added successfully';
            }
            else {
    ...
    public function actionupdatevehiclebyid() {
        ...
        if (count($veh_detail) > 0) {
            ...
            if ($build == 'regular')
                $resUpdate = Yii::app()->db->createCommand()->update('all_vehicles', $data, "id='" . $veh_detail[0]['id'] . "'");
            if ($build == 'classic')
                $resUpdate = Yii::app()->db->createCommand()->update('all_classic_vehicles', $data, "id='" . $veh_detail[0]['id'] . "'");
    ...
    public function actiondeletevehicle() {
        ...
        if ((isset($id) && !empty($id))) {
            ...
            if (!count($veh_detail)) {
                ...
                $result = 'true';
                if ($build == 'regular')
                    Yii::app()->db->createCommand("DELETE FROM all_vehicles WHERE id = " . $id)->execute();
                if ($build == 'classic')
                    Yii::app()->db->createCommand("DELETE FROM all_classic_vehicles WHERE id = " . $id)->execute();
            }
        }
