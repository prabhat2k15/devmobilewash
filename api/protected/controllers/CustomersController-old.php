...
'updated_date' => date("Y-m-d H:i:s")
				...
				if($model->save(false)){
                    ...
                    */
					...
					//echo $from;
if($customers_details->customername && $customers_details->customername != 'N/A') $subject = 'Welcome to Mobile Wash! '.$customername2;
					...
					if($action) $message .= "<a style='background: #076ee1 none repeat scroll 0 0; border: 0 none; border-radius: 5px; color: #fff; cursor: pointer; display: block; font-size: 18px; font-weight: 700;  padding: 12px 0; text-align: center; text-decoration: none; text-transform: uppercase;  width: 283px;' href='https://www.mobilewash.com/email_confirm.php?code=".$encriptpassword."&action=".$action."'> ACTIVATE MY ACCOUNT </a>";

					else $message .= "<a style='background: #076ee1 none repeat scroll 0 0; border: 0 none; border-radius: 5px; color: #fff; cursor: pointer; display: block; font-size: 18px; font-weight: 700;  padding: 12px 0; text-align: center; text-decoration: none; text-transform: uppercase;  width: 283px;' href='https://www.mobilewash.com/email_confirm.php?code=".$encriptpassword."'> ACTIVATE MY ACCOUNT </a>";

					$message .= "<p style='margin-bottom: 0;'><a href='https://www.mobilewash.com/coming-soon/' style='display: inline-block; margin-right: 15px;'><img src='https://www.mobilewash.com/images/app-store-btn-large.png' alt='' width='135' /></a><a style='display: inline-block; margin-right: 15px;' href='https://www.mobilewash.com/coming-soon/'><img src='https://www.mobilewash.com/images/gplay-btn-large.png' alt='' width='135' /></a></p>";
...
if(count($cust_name > 1)) $customername = $cust_name[0]." ".strtoupper(substr($cust_name[1], 0, 1)).".";
	...
	/*
	** Performs the Customer facebook login Registration.
	...
	*/
    ...
    public function actionfacebooklogin(){
		...
		$device_token = Yii::app()->request->getParam('device_token');
        $mobile_type = Yii::app()->request->getParam('mobile_type');
		$profileimg = Yii::app()->request->getParam('profileimg');
$client_position = Yii::app()->request->getParam('client_position');
		...
		if((isset($emailid)) && !empty($emailid)){
			$customers_id = Customers::model()->findByAttributes(array("email" => $emailid,"account_status"=>1));
            $customer_login_status =  Yii::app()->db->createCommand("SELECT * FROM `customers` WHERE `email` = '$emailid' AND `account_status` = 1 ")->queryAll();
            if($customer_login_status[0]['device_token']!=$device_token && $customer_login_status[0]['online_status']=='online')
             ...
             else
             {
                ...
                $latestwash = Washingrequests::model()->findByAttributes(array('customer_id'=>$customers_id->id), array('order'=>'created_date DESC'));

 $allschedwashes = Washingrequests::model()->findAllByAttributes(array('customer_id' => $customers_id->id, 'is_scheduled' => 1), array('condition'=>'status = 0 OR status = 1 OR status = 2'));
...
if($min_diff <= 60 && $min_diff >= 0){
   ...
   break;
				...
				if(!empty($device_token)){
					$model= Customers::model()->findByAttributes(array('id'=>$customers_id->id));
				...
				if(empty($customers_id->customername) || $customers_id->customername=='(null)'){
					$customers_id->customername = $customername;
					$model= Customers::model()->findByAttributes(array('id'=>$customers_id->id));
					$data= array('customername'=>$customername);
					$data= array_filter($data);
					$model->attributes = $data;
					$model->save(false);
				}
...
if(count($cust_name > 1)) $customername = $cust_name[0]." ".strtoupper(substr($cust_name[1], 0, 1)).".";
				...
				$json= array(
					...
					'contact_number' => $customers_id->contact_number,
                    'locations' => $locations,
'fifth_wash_points' => $customers_id->fifth_wash_points,
                    'total_washes' => $customers_id->total_wash,
                    'email_alerts'=> $customers_id->email_alerts,
                    'push_notifications'=> $customers_id->push_notifications,
'phone_verified'=> $customers_id->phone_verified,
'client_position' => !empty($client_position) ? $client_position : '',
'upcoming_schedule_wash_details' => $upcoming_schedule_wash_details
				...
				$customerid= '';
				if((isset($emailid) && !empty($emailid)) && (isset($image) && !empty($image)) && (isset($customername) && !empty($customername))){
...
'updated_date' => date("Y-m-d H:i:s")
					...
					if($model->save(false)){
						...
						if(count($customers_details)>0){
							...
							$email = $customers_details->email;
							$image = $customers_details->image;
							$login_type = $customers_details->login_type;
						}
						$result= 'true';
						$response= 'User successfully registered and login';
						$json= array(
							'result'=> $result,
							'response'=> $response,
							'customerid' => $customers_details->id,
							'email' => $customers_details->email,
							'customername' => $customername2,
							'image' => $customers_details->image,
							'client_position' => !empty($client_position) ? $client_position : '',
							'contact_number' => $customers_details->contact_number,
'fifth_wash_points' => $customers_details->fifth_wash_points,
                            'locations' => '',
                    'total_washes' => 0,
                    'email_alerts'=> 0,
                    'push_notifications'=> 0,
...
'upcoming_schedule_wash_details' => $upcoming_schedule_wash_details
		...
		if(isset($email) && !empty($email)){
			...
			if(count($customers_email_exists)>0){
				...
				if($customers_email_exists->save(false)){
					...
					$reporttxt = 'https://www.mobilewash.com/reset-password.php?action=clrp&token='.$token.'&id='.$customers_email_exists->id;
                    $message = "";
	...
	*/
    public function actionprofileupdate(){
		...
		if(Yii::app()->request->getParam('fifth_wash_points')) $fifth_wash_points = Yii::app()->request->getParam('fifth_wash_points');
...
$dup_address_check = CustomerLocation::model()->findAllByAttributes(array("location_address"=>$location_address, "customer_id" => $customer_id));

             	if(!count($customer_check)){
                   	$result= 'false';
		$response= 'Invalid customer';
                }
                else if(count($location_limit_check) >= 5){
                    	$result= 'false';
		$response= 'Location limit exceeded. You can add upto 5 locations';
                }
                else{
if(!count($dup_address_check)){
                   $locationdata= array(
...
$vehicle_build = '';
			$customer_id = Yii::app()->request->getParam('customer_id');
            if(Yii::app()->request->getParam('vehicle_no')) $vehicle_no = Yii::app()->request->getParam('vehicle_no');
 if(Yii::app()->request->getParam('vehicle_build')) $vehicle_build = Yii::app()->request->getParam('vehicle_build');

			$brand_name = Yii::app()->request->getParam('brand_name');
			$model_name = Yii::app()->request->getParam('model_name');
$vehicle_category = '';
if(Yii::app()->request->getParam('vehicle_category')) $vehicle_category = Yii::app()->request->getParam('vehicle_category');
...
$pet_hair = 0;
if(Yii::app()->request->getParam('pet_hair')) $pet_hair = Yii::app()->request->getParam('pet_hair');
$lifted_vehicle = 0;
if(Yii::app()->request->getParam('lifted_vehicle')) $lifted_vehicle = Yii::app()->request->getParam('lifted_vehicle');
$exthandwax_addon = 0;
if(Yii::app()->request->getParam('exthandwax_addon')) $exthandwax_addon = Yii::app()->request->getParam('exthandwax_addon');
$extplasticdressing_addon = 0;
if(Yii::app()->request->getParam('extplasticdressing_addon')) $extplasticdressing_addon = Yii::app()->request->getParam('extplasticdressing_addon');
$extclaybar_addon = 0;
if(Yii::app()->request->getParam('extclaybar_addon')) $extclaybar_addon = Yii::app()->request->getParam('extclaybar_addon');
$waterspotremove_addon = 0;
if(Yii::app()->request->getParam('waterspotremove_addon')) $waterspotremove_addon = Yii::app()->request->getParam('waterspotremove_addon');
						  ...
						  //exit;
							try{
								$resIns = Yii::app()->db->createCommand()
										->insert('customer_vehicals', array('customer_id'=>$customer_id,
...
file_put_contents("setvehiclestatus.log",$wash_request_id."+".$vehicle_id."+".$status."+".$eco_friendly."+".$damage_points."\n",FILE_APPEND);        if((isset($wash_request_id) && !empty($wash_request_id)) && (isset($vehicle_id) && !empty($vehicle_id)) && (isset($status))) {
 ...
 if($upgrade_pack == 1){
Vehicle::model()->updateByPk($vehicle_id, array('pet_hair' => $pet_hair, 'lifted_vehicle' => $lifted_vehicle, 'new_pack_name' => $new_pack_name, 'exthandwax_addon' => $exthandwax_addon, 'extplasticdressing_addon' => $extplasticdressing_addon, 'extclaybar_addon' => $extclaybar_addon, 'waterspotremove_addon' => $waterspotremove_addon));
...
Washingrequests::model()->updateByPk($wash_request_id, array('waterspotremove_vehicles' => $waterspotremove_addon_new));
                ...
                if($upgrade_pack == 2){
                    ...
                    Washingrequests::model()->updateByPk($wash_request_id, array('package_list' => $updated_packs));

                }

if($upgrade_pack == 3){

Vehicle::model()->updateByPk($vehicle_id, array('pet_hair' => 0, 'lifted_vehicle' => 0, 'new_pack_name' => '', 'exthandwax_addon' => 0, 'extplasticdressing_addon' => 0, 'extclaybar_addon' => 0, 'waterspotremove_addon' => 0));
...
if (!in_array($vehicle_id, $pet_hair_vehicles_arr)) array_push($pet_hair_vehicles_arr, $vehicle_id);
$pet_hair_vehicles_new = implode(",", $pet_hair_vehicles_arr);
$pet_hair_vehicles_new = trim($pet_hair_vehicles_new,",");
Washingrequests::model()->updateByPk($wash_request_id, array('pet_hair_vehicles' => $pet_hair_vehicles_new));
...
if (!in_array($vehicle_id, $lifted_vehicles_arr)) array_push($lifted_vehicles_arr, $vehicle_id);
$lifted_vehicles_new = implode(",", $lifted_vehicles_arr);
$lifted_vehicles_new = trim($lifted_vehicles_new,",");
Washingrequests::model()->updateByPk($wash_request_id, array('lifted_vehicles' => $lifted_vehicles_new));
...
if (!in_array($vehicle_id, $exthandwax_addon_arr)) array_push($exthandwax_addon_arr, $vehicle_id);
$exthandwax_addon_new = implode(",", $exthandwax_addon_arr);
$exthandwax_addon_new = trim($exthandwax_addon_new,",");
Washingrequests::model()->updateByPk($wash_request_id, array('exthandwax_vehicles' => $exthandwax_addon_new));
...
if (!in_array($vehicle_id, $extplasticdressing_addon_arr)) array_push($extplasticdressing_addon_arr, $vehicle_id);
$extplasticdressing_addon_new = implode(",", $extplasticdressing_addon_arr);
$extplasticdressing_addon_new = trim($extplasticdressing_addon_new,",");
Washingrequests::model()->updateByPk($wash_request_id, array('extplasticdressing_vehicles' => $extplasticdressing_addon_new));
...
if (!in_array($vehicle_id, $extclaybar_addon_arr)) array_push($extclaybar_addon_arr, $vehicle_id);
$extclaybar_addon_new = implode(",", $extclaybar_addon_arr);
$extclaybar_addon_new = trim($extclaybar_addon_new,",");
Washingrequests::model()->updateByPk($wash_request_id, array('extclaybar_vehicles' => $extclaybar_addon_new));
...
if (!in_array($vehicle_id, $waterspotremove_addon_arr)) array_push($waterspotremove_addon_arr, $vehicle_id);
$waterspotremove_addon_new = implode(",", $waterspotremove_addon_arr);
$waterspotremove_addon_new = trim($waterspotremove_addon_new,",");
Washingrequests::model()->updateByPk($wash_request_id, array('waterspotremove_vehicles' => $waterspotremove_addon_new));
...
$washing_plan_det = Washingplans::model()->findByAttributes(array("vehicle_type"=>$veh_type, "title"=>$cardata->new_pack_name));
                    if(count($washing_plan_det)) $wash_price = $washing_plan_det->price;
                    else $wash_price = '';
}
else{
...
if((count($car_arr) > 1) && (!$fifth_fee) && (!$first_fee)) {
$bundle_fee = 1;
...
$hours = floor($wash_time / 60);
                   ...
                   $vehicles[] = array("id"=> $carid, "make"=>$cardata->brand_name, "model"=>$cardata->model_name, "license_no"=>$cardata->vehicle_no, "vehicle_type"=>$cardata->vehicle_type, "vehicle_category"=>$cardata->vehicle_category, "vehicle_build"=>$cardata->vehicle_build, "vehicle_image"=>$cardata->vehicle_image, "handling_fee"=>'1.00', "status"=>$cardata->status, "eco_friendly"=>$cardata->eco_friendly, "damage_points"=>$cardata->damage_points,"damage_pic"=>$cardata->damage_pic, "pet_hair"=>number_format($cardata->pet_hair, 2), "pet_hair_agent"=> number_format(round($cardata->pet_hair * .80, 2), 2), "lifted_vehicle"=>number_format($cardata->lifted_vehicle, 2), "lifted_vehicle_agent"=>number_format(round($cardata->lifted_vehicle * .80, 2), 2), "exthandwax_addon"=>number_format($cardata->exthandwax_addon, 2), "exthandwax_addon_agent"=>number_format(round($cardata->exthandwax_addon*.80, 2), 2),"extplasticdressing_addon"=>number_format($cardata->extplasticdressing_addon, 2), "extplasticdressing_addon_agent"=>number_format(round($cardata->extplasticdressing_addon*.80, 2), 2), "extclaybar_addon"=>number_format($cardata->extclaybar_addon, 2), "extclaybar_addon_agent"=>number_format(round($cardata->extclaybar_addon*.80, 2), 2), "waterspotremove_addon"=>number_format($cardata->waterspotremove_addon, 2), "waterspotremove_addon_agent"=>number_format(round($cardata->waterspotremove_addon*.80, 2), 2), "bundle_discount" => number_format($bundle_fee, 2), "bundle_discount_agent" => number_format($bundle_fee_agent, 2), "fifth_wash_discount" => number_format($fifth_fee, 2), "first_wash_discount" => number_format($first_fee, 2), "upgrade_pack"=> $cardata->upgrade_pack, "new_pack_name"=> $cardata->new_pack_name, "edit_vehicle"=> $cardata->edit_vehicle, "remove_vehicle_from_kart"=> $cardata->remove_vehicle_from_kart, "payment_type"=>$pack_arr[$ind], "price"=>$wash_price, "total_price"=>number_format($total_car_price, 2), "price_agent"=>$car_price_agent, "total_price_agent"=>number_format($total_car_price_agent, 2), "wash_time"=> $wash_time, "wash_time_str" => $washtime_str);
                }
              $result = true;
              $response = "Vehicles";
...
// if everything is ok, try to upload file
            ...
            if (move_uploaded_file($_FILES["sample_image"]["tmp_name"], $target_file)) {
               //array("image"=>"http://demo.thetatechnolabs.com/mobilewash/api/".$new_image_name);
                echo json_encode(array("image"=>"http://mobilewash.com/api/uploads/".$new_image_name,"username"=>$username));
            } else {
                echo json_encode(array("error"=>"Sorry, there was an error uploading your file."));
            }
        }
...
if($customers->client_position == 'real'){
				  $createmethodresult = Yii::app()->braintree->updatePaymentMethod_real($payment_method_token,
				['options' => [
					 'makeDefault' => true
				  ]]
				);
 ...
 $createmethodresult = Yii::app()->braintree->updatePaymentMethod($payment_method_token,
				['options' => [
					 'makeDefault' => true
				  ]]
				);
        ...
        public function actionaddwashrequesttransactionid() {
         ...
         public function actiongetcustomerpaymentmethods() {
            ...
            if((isset($customer_id) && !empty($customer_id))){
                ...
                if($customer_exists->client_position == 'real') $Bresult = Yii::app()->braintree->getCustomerById_real($braintree_id);
...
$total_pages = 0;
 ...
 $all_wash_requests_count = Yii::app()->db->createCommand()
			->select('*')
			->from('washing_requests')
			->where("customer_id='".$customer_id."' AND ((status='4' OR status='5' OR status='6' OR is_scheduled = '1') AND no_washer_cancel = 0)", array())
            ->order(array('created_date desc'))
			->queryAll();

$total_entries = count($all_wash_requests_count);
if($total_entries) {
...
$min_diff = round(($from_time - $to_time) / 60,2);
        ...
        $wash_requests[$index]['total'] =$total;
         $wash_requests[$index]['bundle_discount'] =$jsondata->bundle_discount;
         $wash_requests[$index]['fifth_wash_discount'] =$jsondata->fifth_wash_discount;
         $wash_requests[$index]['first_wash_discount'] =$jsondata->first_wash_discount;
if($wrequest['coupon_discount']) $wash_requests[$index]['coupon_discount'] = $wrequest['coupon_discount'];
else $wash_requests[$index]['coupon_discount'] =$jsondata->coupon_discount;
$wash_requests[$index]['coupon_code'] = $wrequest['coupon_code'];
$wash_requests[$index]['tip_amount'] = $wrequest['tip_amount'];
$wash_requests[$index]['customer_wash_points'] =$jsondata->customer_wash_points;
$wash_requests[$index]['cancel_fee'] =$jsondata->cancel_fee;
...
Vargas::Obj()->SendMail("admin@mobilewash.com","info@mobilewash.com",$message,"Customer App Feedback", 'mail-receipt');
...
$jsonpending['created_date'] = $pendings_orders->created_date;
         ...
         //schedule orders
		$schedule_orders = Washingrequests::model()->findAllByAttributes(array("status"=>'0', "is_scheduled"=>'1', 'wash_request_position' => 'real'));
...
$package_price = 0;
            ...
            $wash_package = '';
            if(Yii::app()->request->getParam('vehicle_no')) $vehicle_no = Yii::app()->request->getParam('vehicle_no');
            if(Yii::app()->request->getParam('wash_package')) $wash_package = Yii::app()->request->getParam('wash_package');
             if(Yii::app()->request->getParam('vehicle_category')) $vehicle_category = Yii::app()->request->getParam('vehicle_category');
 if(Yii::app()->request->getParam('vehicle_build')) $vehicle_build = Yii::app()->request->getParam('vehicle_build');
			$brand_name = Yii::app()->request->getParam('brand_name');
			$model_name = Yii::app()->request->getParam('model_name');
...
$fifth_points ++;
if($fifth_points > 5) $fifth_points = 1;
}
}
...
if($total_cars > 0 && (!$fifth_fee) && (!$first_fee)) {
$bundle_fee = 1;
$total_car_price -= $bundle_fee;
}
...
if($vehicle_exists->lifted_vehicle > 0) $wash_time += 5;
if($vehicle_exists->exthandwax_addon > 0) $wash_time += 10;
if($vehicle_exists->extplasticdressing_addon > 0) $wash_time += 5;
if($vehicle_exists->extclaybar_addon > 0) $wash_time += 15;
if($vehicle_exists->waterspotremove_addon > 0) $wash_time += 10;
...
$how_hear_mw = Yii::app()->request->getParam('how_hear_mw');
        ...
        $data = array('customername'=> $customername,'email'=> $email,'contact_number'=> $contact_number,'email_alerts'=> $email_alerts,'push_notifications'=> $push_notifications,'total_wash'=> $total_wash,'time_zone'=> $time_zone,'updated_date'=> $created_date,'updated_date'=> $updated_date,'online_status'=> $online_status,'rating'=> $rating,'image'=> $image, 'password'=> $password, 'how_hear_mw' => $how_hear_mw);
        $data = array_filter($data);
     ...
     public function actionCustomerCancelWashPaymentold() {
            ...
            if((isset($customer_id) && !empty($customer_id)) && (isset($washing_request_id) && !empty($washing_request_id)) && (isset($nonce) && !empty($nonce)) && (isset($amount) && !empty($amount))){
                ...
                $resUpdate = $washrequestmodel->updateAll($data, 'id=:id', array(':id'=>$washing_request_id));
                        ...
                        //print_r($result);die;
                        $response = "Payment successful and wash cancelled";
                        $result = "true";
                        $payment_type = $Bresult['transaction_id'];
                        //$update_request = Washingrequests::model()->findByPk($washing_request_id);
                        //$update_request->transaction_id = $Bresult['transaction_id'];
	...
	*/
	...
	if((isset($customer_id) && !empty($customer_id)) && (isset($latitude) && !empty($latitude)) && (isset($longitude) && !empty($longitude))){
		...
		$customerloc = new CustomerLiveLocations;
			...
			if(!count($checkcustomer)) $resUpdate = $customerloc->save(false);
			else $resUpdate = $customerloc->updateAll($data, 'customer_id=:customer_id', array(':customer_id'=>$customer_id));
			if($resUpdate){
				$result= 'true';
...
// order map for year
    public function Actioncustomeryearwiseorder (){
        ...
        for ($i = 0; $i <=6; $i++) {
            ...
            $request = Yii::app()->db->createCommand("SELECT COUNT(*) as cnt FROM `washing_requests` WHERE created_date BETWEEN '$start_year' AND '$end_year'")->queryAll();
...
$date = date("d", strtotime($i." days ago"));
            $request = Yii::app()->db->createCommand("SELECT COUNT(*) as cnt FROM `washing_requests` WHERE created_date BETWEEN '$start_date' AND '$end_date' ")->queryAll();
            //echo "SELECT COUNT(*) as cnt FROM `washing_requests` WHERE created_date BETWEEN '$start_year' AND '$end_year' " .'<br />';
            if(!empty($request)){
            foreach($request as $details){
...
for ($i = 6; $i >=0; $i--) {
    ...
    $lastdate = date('Y-m-d',$last).' '.'23:59:59';
        ...
        $request = Yii::app()->db->createCommand("SELECT COUNT(*) as cnt FROM `washing_requests` WHERE created_date BETWEEN '$irstdate' AND '$lastdate' ")->queryAll();
 ...
 // new customer for Year
    public function Actioncustomeryearwise (){
        ...
        for($i = 6; $i >=0; $i--){
            ...
            $start_year =  $year.'-01-01'.' '.'00:00:00';
            $end_year =  $year.'-12-31'.' '.'23:59:59';
            $request = Yii::app()->db->createCommand("SELECT COUNT(*) as cnt FROM `customers` WHERE created_date BETWEEN '$start_year' AND '$end_year' AND client_position='real'")->queryAll();
            //echo "SELECT COUNT(*) as cnt FROM `washing_requests` WHERE created_date BETWEEN '$start_year' AND '$end_year' " .'<br />';
            foreach($request as $details){
                $array[$year] = $details['cnt'];
            }
...
$day = date("D", strtotime($i." days ago"));
            $request = Yii::app()->db->createCommand("SELECT COUNT(*) as cnt FROM `customers` WHERE created_date BETWEEN '$start_date' AND '$end_date' AND client_position='real'")->queryAll();
            //echo "SELECT COUNT(*) as cnt FROM `washing_requests` WHERE created_date BETWEEN '$start_year' AND '$end_year' " .'<br />';
            foreach($request as $details){
                $array[$day] = $details['cnt'];
            }
...
for($i = 6; $i >=0; $i--){
    ...
    $lastdate = date('Y-m-d',$last).' '.'23:59:59';
        ...
        $request = Yii::app()->db->createCommand("SELECT COUNT(*) as cnt FROM `customers` WHERE created_date BETWEEN '$irstdate' AND '$lastdate'  AND client_position='real'")->queryAll();
        //echo "SELECT COUNT(*) as cnt FROM `customers` WHERE created_date BETWEEN '$irstdate' AND '$lastdate' ".$month.'<br />';
...
// new pre clients in a month
    public function Actionpreclientsmonthwise (){
		...
		for ($i = 6; $i >=0; $i--) {
			...
			file_put_contents($path,"SELECT COUNT(*) as cnt FROM `customers` WHERE created_date BETWEEN '$irstdate' AND '$lastdate' "); */
 ...
 public function actionGetAllActiveWashersTrashData(){
  ...
  public function ActionMovePreToRealClient (){
         ...
         $update_user = Yii::app()->db->createCommand("UPDATE pre_registered_clients SET trash_status='1' WHERE id = '$client_id' ")->execute();
                    ...
                    $reporttxt = 'https://www.mobilewash.com/set-password.php?action=clrp&id='.$insertid;
                    $message = "";
                ...
                if(!$customers){
					...
					if($cust_bt_id){
						if($payment_method_token){
							...
							['options' => [
								'verifyCard' => false,
								'verificationMerchantAccountId' => 'mobilewashinc',
...
$kartdetails = json_decode($kartresult);
             ...
             $request_data = ['merchantAccountId' => $agent_check->bt_submerchant_id, 'serviceFeeAmount' => $kartdetails->company_total, 'amount' => $kartdetails->net_price,'paymentMethodToken' => $token, 'options' => ['submitForSettlement' => True]];
                    ...
                    if($payresult['success'] == 1) {
                        ...
                        Washingrequests::model()->updateByPk($wash_request_id, array('transaction_id' => $payresult['transaction_id'], 'payment_type' => 'Credit Card'));
...
print_r($Bresult);
	...
	public function actionReview(){
		...
		/* $path = '/home/mobilewa/public_html/api/protected/controllers/test.php';
		file_put_contents($path, print_r($all_review,true)); */
	}
