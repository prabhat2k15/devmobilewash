...
$client_position = APP_ENV; // change it to blank in dev and real to live
$login_type = Yii::app()->request->getParam('login_type');
$login_type = 'facebook';
...
'upcoming_schedule_wash_details' => $upcoming_schedule_wash_details
				...
				if((isset($emailid) && !empty($emailid)) && (isset($image) && !empty($image)) && (isset($customername) && !empty($customername))){
                 ...
                 $clients_email_exists = Customers::model()->findByAttributes(array("email"=>$emailid));
                  	$agents_email_exists = Agents::model()->findByAttributes(array("email"=>$emailid));
		...
		if(isset($email) && !empty($email)){
			...
			if(count($customers_email_exists) &&  ($customers_email_exists->id != $customer_id)){
			 ...
			 $customername2 = '';
					...
					$from = Vargas::Obj()->getAdminFromEmail();
					//echo $from;
if($customers_email_exists->customername && $customers_email_exists->customername != 'N/A') $subject = 'Welcome to Mobile Wash! '.$customername2;
else $subject = 'Welcome to Mobile Wash!';
					//$message = "Hello ".$customername2.",<br/><br/>Welcome to Mobile wash!";

                    if($customers_email_exists->customername && $customers_email_exists->customername != 'N/A') $message = "<h1>Hello ".$customername2."!</h1>";
...
if (!in_array($qrVehicles['id'], $pet_hair_vehicles_arr)) array_push($pet_hair_vehicles_arr, $qrVehicles['id']);
...
if (!in_array($qrVehicles['id'], $lifted_vehicles_arr)) array_push($lifted_vehicles_arr, $qrVehicles['id']);
...
if (!in_array($qrVehicles['id'], $exthandwax_addon_arr)) array_push($exthandwax_addon_arr, $qrVehicles['id']);
...
if (!in_array($qrVehicles['id'], $extplasticdressing_addon_arr)) array_push($extplasticdressing_addon_arr, $qrVehicles['id']);
...
if (!in_array($qrVehicles['id'], $extclaybar_addon_arr)) array_push($extclaybar_addon_arr, $qrVehicles['id']);
...
if (!in_array($qrVehicles['id'], $waterspotremove_addon_arr)) array_push($waterspotremove_addon_arr, $qrVehicles['id']);
...
else $wash_requests[$index]['coupon_discount'] =0;
...
if($vehicle_exists->wash_package == 'Premium'){
   $wash_price += $surgeprice[0]['premium'];
   $wash_price = (string) $wash_price;
}
...
'pet_hair' => $vehicle_exists->pet_hair,
'lifted_vehicle' => $vehicle_exists->lifted_vehicle,
'exthandwax_addon' => $vehicle_exists->exthandwax_addon,
'extplasticdressing_addon' => $vehicle_exists->extplasticdressing_addon,
'extclaybar_addon' => $vehicle_exists->extclaybar_addon,
'waterspotremove_addon' => $vehicle_exists->waterspotremove_addon,
"handling_fee"=>'1.00',
...
die();
		...
		$limit = Yii::app()->request->getParam('limit');
           ...
           if(!$limit) $customers = Customers::model()->findAll(array('order'=>'created_date desc'));
else $customers = Customers::model()->findAll(array('order'=>'created_date desc', 'limit' => $limit));
...
$kartdetails = json_decode($kartapiresult);
				...
				{
				    if($customer_check->client_position == 'real') $voidresult = Yii::app()->braintree->void_real($wash_check->transaction_id);
               ...
               $request_data = ['merchantAccountId' => 'al_davi_instant_4pjkk25r', 'serviceFeeAmount' => $company_total, 'amount' => $total,'paymentMethodToken' => $token];
                ...
                $request_data = ['merchantAccountId' => 'mobilewash_payment_inst_m59bj2b6', 'serviceFeeAmount' => $company_total, 'amount' => $total,'paymentMethodToken' => $token];
                    ...
                    if($payresult['success'] == 1) {
                        ...
                        if($customer_check->client_position == 'real') Yii::app()->braintree->updatePaymentMethod_real($token, ['options' => ['makeDefault' => true]]);
                        else Yii::app()->braintree->updatePaymentMethod($token, ['options' => ['makeDefault' => true]]);
 ...
 public function actioncustomerupfrontpayment_new(){
    ...
    if((isset($customer_id) && !empty($customer_id)) && (isset($total) && !empty($total)) && (isset($company_total) && !empty($company_total))){
		...
		/* else if((!$is_scheduled) && ($nearagentsdetails->result == 'false')){
			...
			echo '<pre>';print_r($creditCard);die;
...
die();
            ...
            die();
                ...
                if(($wash_position == 'demo') || ($wash_position == '')) $Bresult = Yii::app()->braintree->getCustomerById($customers->braintree_id);
                else $Bresult = Yii::app()->braintree->getCustomerById_real($customers->braintree_id);
                $token = '';
                  if(count($Bresult->paymentMethods)){
                    foreach($Bresult->paymentMethods as $index=>$paymethod){
                         $payment_methods[$index]['title'] = get_class($paymethod);
