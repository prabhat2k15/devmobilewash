    ...
    public function washingkart($wash_request_id, $api_key, $coupon_discount = 0, $api_password = '', $api_token = '', $t1 = '', $t2 = '', $user_type = '', $user_id = '', $show_payment_method = '') {
        ...
        if ((isset($wash_request_id) && !empty($wash_request_id))) {
            ...
            if (!count($wash_id_check)) {
                ...
                if ($pet_hair_vehicles_custom_amount)
                    $pet_hair_vehicles_custom_amount_obj = json_decode($pet_hair_vehicles_custom_amount);
                if ($wash_id_check->coupon_discount)
                    $coupon_discount = $wash_id_check->coupon_discount;
                if ($wash_id_check->coupon_code)
                    $coupon_code = $wash_id_check->coupon_code;
...
//var_dump($geojsondata);
                        ...
                        foreach ($car_addon_price as $addonprice) {
                            if ($addonprice['title'] == 'Clay Bar')
                                $extclaybar_price = $addonprice['price'];
                            if ($addonprice['title'] == 'Lifted Truck')
                                $lift_price = $addonprice['price'];
                            if ($addonprice['title'] == 'Exterior Hand Wax')
                                $exthandwax_price = $addonprice['price'];
                            if ($addonprice['title'] == 'Exterior Dressing')
                                $extplasticdressing_price = $addonprice['price'];
                            if ($addonprice['title'] == 'Water Spot')
                                $waterspotremove_price = $addonprice['price'];
                            if ($addonprice['title'] == 'Upholstery Conditioning')
                                $upholstery_price = $addonprice['price'];
                            if ($addonprice['title'] == 'Floor Mat Cleaning')
                                $floormat_price = $addonprice['price'];
                        }
                        ...
                        if (in_array($car, $extclaybar_vehicles_arr)) {
                            ...
                            if (count($vehicle_wash_pricing)) {
                                ...
                                if ($extclaybar_price) {
                                    ...
                                    $clay_price = 40;
                                    if ($vehicle_details->vehicle_type == 'S')
                                        $clay_price = 40;
                                    if ($vehicle_details->vehicle_type == 'M')
                                        $clay_price = 42.50;
                                    if ($vehicle_details->vehicle_type == 'L')
                                        $clay_price = 45;
                                    if ($vehicle_details->vehicle_type == 'E')
                                        $clay_price = 45;
                                }
                        ...
                        if ((!$fifth_wash_disc) && (!$first_wash_discount) && (count($total_cars) > 1)) {
                            if (count($vehicle_wash_pricing))
                                $bundle_disc = $vehicle_wash_pricing->bundle_disc;
                            else
                                $bundle_disc = 1;
                            $bundle_discount++;
                        }
                        ...
                        if ((count($total_cars) > 1) && ($carindex == 0) && ($wash_id_check->coupon_discount > 0))
                            $bundle_disc = 0;
                ...
                if (($wash_id_check->coupon_discount > 0) && $bundle_discount)
                    $bundle_discount -= 1;
                ...
                if ($coupon_discount > 0) {
                    ...
                    if (count($total_cars) > 1)
                        $company_total -= $coupon_discount - .8;
                    else
                ...
                if ($wash_id_check->transaction_id) {
                    if ($wash_id_check->wash_request_position == 'real')
                        $Bresult = Yii::app()->braintree->getTransactionById_real($wash_id_check->transaction_id);
                    else
                        $Bresult = Yii::app()->braintree->getTransactionById($wash_id_check->transaction_id);

                    if ($Bresult['transaction_id']) {
                        if ($Bresult['card_no_last4'])
                            $card_no = "************" . $Bresult['card_no_last4'];
                        else
                            $card_no = '';
                    ...
                    if ($show_payment_method == 'true') {
                        ...
                        if ($cust_details->client_position == 'real')
                            $Bresult = Yii::app()->braintree->getCustomerById_real($cust_details->braintree_id);
                        else
                            $Bresult = Yii::app()->braintree->getCustomerById($cust_details->braintree_id);
                        //var_dump($Bresult);
        ...
        if ((AES256CBC_STATUS == 1) && ($api_password != AES256CBC_API_PASS)) {
            ...
            $json = array(
                ...
                'waived_fee' => number_format($wash_id_check->waived_fee, 2),
    ...
    public function verifyapitemptoken($string, $key = '', $iv = '', $password = '') {
        ...
        $gettemptokens = Yii::app()->db->createCommand("SELECT * FROM temp_tokens ORDER BY id DESC")->queryAll();
    ...
    public function verifyapitoken($string, $key = '', $iv = '', $user_type = '', $user_id = '', $password = '') {
        if ($password != AES256CBC_API_PASS) {
            echo "Access denied";
            die();
        }
        ...
        if ($user_type == 'customer')
            $checkuser = Yii::app()->db->createCommand("SELECT id, access_token FROM customers WHERE id=" . $user_id)->queryAll();
        if ($user_type == 'washer')
            $checkuser = Yii::app()->db->createCommand("SELECT id, access_token FROM agents WHERE id=" . $user_id)->queryAll();
        if ($user_type == 'admin')
            $checkuser = Yii::app()->db->createCommand("SELECT id, access_token FROM users WHERE id=" . $user_id)->queryAll();
        ...
        foreach ($checkuser as $user) {
            $getsavedtoken = openssl_decrypt(base64_decode($user['access_token']), "AES-128-CBC", AES128CBC_KEY, $options = OPENSSL_RAW_DATA, AES128CBC_IV);
