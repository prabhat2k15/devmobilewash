...
if (!empty($_POST['hidden'])) {
    $data = array('id' => 1, 'first_wash_discount' => $_POST['first_wash_status'], 'key' => API_KEY, 'api_token' => $finalusertoken, 't1' => $mw_admin_auth_arr[2], 't2' => $mw_admin_auth_arr[3], 'user_type' => 'admin', 'user_id' => $mw_admin_auth_arr[4]);
...
$data = '';
curl_setopt($handle, CURLOPT_POST, true);
curl_setopt($handle, CURLOPT_POSTFIELDS, array('id' => 1, 'key' => API_KEY, 'api_token' => $finalusertoken, 't1' => $mw_admin_auth_arr[2], 't2' => $mw_admin_auth_arr[3], 'user_type' => 'admin', 'user_id' => $mw_admin_auth_arr[4]));
curl_setopt($handle, CURLOPT_RETURNTRANSFER, 1);
$result = curl_exec($handle);
curl_close($handle);
        ...
        <div class="caption">
            ...
            if (!empty($msg)) {
                if ($msg == 'Settings updated successfully') {
                    ?> <p style="padding: 10px; background: green; color: #fff;"><?php echo $msg; ?></p> <?php } else { ?>
