<?php
    if(!empty($_GET['action'])){
        $clientsid = $_GET['id'];
        $url = 'http://www.devmobilewash.com/api/index.php?r=customers/restorepreclients&id='.$clientsid;
        ...
        if($response == "clients restore" && $result_code == "true"){
            ?>
            <script type="text/javascript">window.location = "http://www.devmobilewash.com/admin-new/manage-pre-clients.php?restore=true"</script>
            <?php
            die();
            }
    }
...
<?php
       ...
       $url = 'http://www.devmobilewash.com/api/index.php?r=customers/getalltrashpreclients'; 
            $handle = curl_init($url);
            $data = array('key' => 'Tva4hwH9KvqEQHTz5nHZTLhAV7Bv68AAtBeAHMA4');
            curl_setopt($handle, CURLOPT_POST, true);
            curl_setopt($handle, CURLOPT_POSTFIELDS, $data);
