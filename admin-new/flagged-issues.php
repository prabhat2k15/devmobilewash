...
$url = ROOT_URL . '/api/index.php?r=site/getallFlaggedIssues';
$cust_id = 0;
$agent_id = 0;
...
<?php
if (isset($_POST['submitFlagged'])) {
    if ($_POST['submitFlagged'] == "TODO") {
        $val = 1;
    }
    if ($_POST['submitFlagged'] == "Resolved") {
        $val = 2;
    }
    $resolvedValue = implode(',', $_POST['resolvedValue']);
    $FlaggedIssueData = array("resolvedValue" => $resolvedValue, 'flagVal' => $val, 'key' => API_KEY, 'api_token' => $finalusertoken, 't1' => $mw_admin_auth_arr[2], 't2' => $mw_admin_auth_arr[3], 'user_type' => 'admin', 'user_id' => $mw_admin_auth_arr[4]);
    $handle_data = curl_init(ROOT_URL . "/api/index.php?r=FlaggedIssue/UpdateFlaggedIssueMultiple");
    curl_setopt($handle_data, CURLOPT_POST, true);
    curl_setopt($handle_data, CURLOPT_POSTFIELDS, $FlaggedIssueData);
    curl_setopt($handle_data, CURLOPT_RETURNTRANSFER, 1);
    $result = curl_exec($handle_data);
    curl_close($result);
    curl_close($handle_data);
    $updateResponse = json_decode($result);
    $url = ROOT_URL . '/admin-new/flagged-issues.php?filter=flaggedIssues&limit=' . $_GET['limit'] . '&flaggedIssueStatus=';
    header('Location:' . $url);
}
?>
<style>
                        ...
                        <div class="caption font-dark">
                            ...
                            <span class="caption-subject bold uppercase"> <?php
                                ...
                                if ($_GET['filter'] == 'nonupcoming')
                                    echo 'Non-Upcoming';
                                ?> Flagged Orders </span><?php if ((isset($_GET['event'])) && ($_GET['event'] == 'total_orders' || $_GET['event'] == 'newcustomer')) { ?><a style="margin-left: 20px;" class="csv-link" href="javascript:void(0)">Download CSV</a>
                                <?php } ?>
                        </div>
                        <div class="caption font-dark">
...
<!--                                        <li><input type="checkbox" id="checkAll">Select All</li>-->
                                ...
                                <div class="col-xs-6 col-sm-4 col-md-3">
                                    <p style="margin-bottom: 20px; font-size: 16px;"><span class="pull-right">IOS Order <?php echo $ios_count; ?></span></p>
                                </div>
                            </div>
                            ...
                            <table class="table table-striped table-bordered table-hover table-checkable order-column" id="example1">
                                <thead>
                                    <tr>
                                        <th style="min-width:140px;">Flagged Action</th>
                                        <th> Actions </th>
                                        <th> ID </th>
                                        <th> Order Type </th>
                                        <th> Status </th>
                                ...
                                <tbody>
                                    ...
                                    <?php
                                    foreach ($s_mw_all_orders as $ind => $order) {
                                        ?>
                                        <tr class="odd gradeX <?php if ($ind == 0) echo "flashro"; ?>" id="order-<?php echo $order->id; ?>">
                                            <td>

                                                <?php if ($order->flagged_issue_status == 2) { ?>
                                                    <input type="button" value="Reopen"  name="submitFlagged1" id="<?= $order->id ?>" data-flag="1" class="btn btn-primary btn-resolved-grn bold-txt buttonSubmit submitFlaggedIssue">

                                                <?php } ?>
                                                <?php if ($order->flagged_issue_status == 1) { ?>
                                                    <input type="button" value="Resolve"  name="submitFlagged1" id="<?= $order->id ?>" data-flag="2" class="btn btn-danger bold-txt buttonSubmit submitFlaggedIssue">
...
<?php }; ?>
    ...
    $(function () {
        ...
        var curr_url = "<?php echo ROOT_URL; ?>/admin-new/flagged-issues.php?filter=<?php echo $_GET['filter']; ?>&flaggedIssueStatus=<?php echo $_GET['flaggedIssueStatus']; ?>";
...
<script>
    ...
    //     }
    // });
    // $('.countSelectBox').on('click', function () {
    //     var count = $(".countSelectBox:checked").length;
    //     if (count == 0) {
    //         $('.buttonSubmit').attr('disabled', true);
    //     } else {
    ...
    //     var count = $(".countSelectBox:checked").length;
    //     if (this.checked) {
    //         $('.countSelectBox').each(function () { //loop through each checkbox
    ...
    $(".submitFlaggedIssue").click(function () {
        var id = $(this).attr('id');
        ...
        $.ajax({
            ...
            type: "GET",
            url: "<?php echo ROOT_URL; ?>/api/index.php?r=FlaggedIssue/UpdateFlagIssue&orderId=" + id + "&flaggedVal=" + flagVal + "&user_name=<?= $jsondata_permission->user_name ?>",
